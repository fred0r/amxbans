...
if($cookie_tmp != "" && $_SESSION["loggedin"]==false) {
	...
	if(count($cook) >= 2) {
		$sid = sql_safe($cook[0]);
		$lang = sql_safe($cook[1]);
		
		$stmt = $mysql->prepare("SELECT id,username,level,email,session_token FROM `".$config->db_prefix."_webadmins` WHERE logcode=? LIMIT 1");
		...
		if($query->num_rows) {
			...
			$stored_token = $result->session_token;
			$provided_token = isset($cook[2]) ? sql_safe($cook[2]) : '';
...
if($cookie_tmp != "" && $_SESSION["loggedin"]==true && isset($_SESSION["uid"])) {
	$stmt = $mysql->prepare("UPDATE `".$config->db_prefix."_webadmins` SET `last_action`=UNIX_TIMESTAMP() WHERE `id`=?");
