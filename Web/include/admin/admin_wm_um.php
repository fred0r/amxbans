...
if(isset($_POST["del"]) && $_SESSION["loggedin"]) {
	$query = $mysql->query("DELETE FROM `".$config->db_prefix."_usermenu` WHERE `id`=".$mid." LIMIT 1") or die ($mysql->error);
...
if(isset($_POST["new"]) && $_SESSION["loggedin"]) {
	$query = $mysql->query("INSERT INTO `".$config->db_prefix."_usermenu` 
							(`pos`,`activ`,`url`,`lang_key`,`url2`,`lang_key2`) 
							VALUES 
							(".(int)$_POST["pos"].",1,
							'".$mysql->escape_string($_POST["url"])."',
							'".$mysql->escape_string($_POST["lang_key"])."',
							'".$mysql->escape_string($_POST["url2"])."',
							'".$mysql->escape_string($_POST["lang_key2"])."'
							)") or die ($mysql->error);
...
if(isset($_POST["save"]) && $_SESSION["loggedin"]) {
		...
		$query = $mysql->query("UPDATE `".$config->db_prefix."_usermenu` SET 
					`activ`=".(isset($_POST["activ"])?1:0).",
					`url`='".$mysql->escape_string($_POST["url"])."',
					`lang_key`='".$mysql->escape_string($_POST["lang_key"])."',
					`url2`='".$mysql->escape_string($_POST["url2"])."',
					`lang_key2`='".$mysql->escape_string($_POST["lang_key2"])."'
