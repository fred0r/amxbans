...
*/
	...
	if(!$_SESSION["loggedin"] || $_SESSION['webadmins_edit']!="yes") {
	...
	function checkAdmin($nickname, $email) {
		global $config, $mysql;
		$stmt = $mysql->prepare("SELECT * FROM `".$config->db_prefix."_webadmins` WHERE username=? OR email=?");
	...
	if(isset($_POST["setnewpw"]) && ($_SESSION['webadmins_edit'] || $_SESSION["uid"] == $uid)) {
		...
		if(!$user_msg) {
			...
			$stmt = $mysql->prepare("UPDATE `".$config->db_prefix."_webadmins` SET `password`=? WHERE `id`=? LIMIT 1");
			$stmt->bind_param("si", $hashed_pw, $uid);
			$stmt->execute();
			if(!$user_msg) {
	...
	if(isset($_POST["save"]) && $_SESSION['webadmins_edit']) {
		if(!$user_msg) {
			$stmt = $mysql->prepare("UPDATE `".$config->db_prefix."_webadmins` SET `username`=?, `level`=?, `email`=?, `logcode`='', `session_token`=NULL WHERE `id`=? LIMIT 1");
			$stmt->bind_param("sisi", $name, $_POST["level"], $email, $uid);
			$stmt->execute();
	...
	if(isset($_POST["del"]) && $_SESSION['webadmins_edit']) {
		$stmt = $mysql->prepare("DELETE FROM `".$config->db_prefix."_webadmins` WHERE `id`=? LIMIT 1");
	...
	if(isset($_POST["new"]) && $_SESSION['webadmins_edit']) {
		...
		if(!$user_msg) {
			//save webadmin to db with bcrypt password
			$hashed_pw = password_hash($pw, PASSWORD_BCRYPT);
			$stmt = $mysql->prepare("INSERT INTO `".$config->db_prefix."_webadmins` (`username`,`password`,`level`,`email`) VALUES (?,?,?,?)");
			$stmt->bind_param("ssis", $name, $hashed_pw, $level, $email);
			$stmt->execute() or $user_msg='_WADMINADDEDFAILED';
			if(!$user_msg) {
