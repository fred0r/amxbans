			...
			<table width="90%" cellspacing="0">
				...
				{foreach from=$ban_details_activ item=ban_detail_activ}
					<tr style="cursor:pointer;" onClick="NewToggleLayer('layer_{$ban_detail_activ.bid}');" class="list">
						...
						<td align="center">{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$ban_detail_activ.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td>
						...
						<td class="table_list" colspan=10>
							<center>
								...
								<table width="90%" class="table_details" cellspacing="0">
									...
									<tr><td><b>{"_BANBY"|lang}:</b></td><td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$ban_detail_activ.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td></tr>
			...
			<table width="90%" cellspacing="0">
				...
				{foreach from=$ban_details_exp item=ban_detail_exp}
					<tr class="list"  style="cursor:pointer;" onClick="NewToggleLayer('layer_{$ban_detail_exp.bid}');" >
						...
						<td align="center">{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$ban_detail_exp.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td>
						...
						<td colspan=10>
							<div style="display: none">
								...
								<table width="90%" class="table_details" cellspacing="0">
									...
									<tr><td><b>{"_BANBY"|lang}:</b></td><td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$ban_detail_exp.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td></tr>
