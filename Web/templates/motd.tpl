...
<body>
	...
	<table align="center" border="1" width="99%" class="table_list" cellpadding="3">
		...
		<tr><td><b>{"_BANBY"|lang}:</b></td><td>{if $show_admin==1 || $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$ban_detail.admin_nick}{if $ban_detail.nickname} <i>({$ban_detail.nickname})</i>{/if}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td></tr>
	...
	<table align="center" border="1" width="99%" class="table_list" cellpadding="3">
		...
		{if $exp_count}
			<tr class="admin_head1">
				...
				{if $show_admin==1 || $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}<td width="1%" nowrap><b>{"_BANBY"|lang}</b></td>{/if}
			</tr>
			{foreach from=$exp_bans item=exp_ban}
				<tr>
					<td align="center" nowrap><b>{$exp_ban.ban_created|date_format:"%d.%m.%Y"}</b></td>
					<td align="center" nowrap>{if $exp_ban.ban_length==0}{"_PERMANENT"|lang}{elseif $exp_ban.ban_length==-1}{"_UNBANNED"|lang}{else} {($exp_ban.ban_length*60)|date2word:true}{/if}</td>
					<td nowrap>{$exp_ban.ban_reason}</td>
					{if $show_admin==1 || $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}<td align="center" nowrap>{$exp_ban.admin_nick}</td>{/if}
				</tr>
			{/foreach}
		{else}
			<tr>
