		...
		<table frame="box" rules="groups" summary=""> 
			...
			<tbody> 
				...
				<tr> 
					...
					<td><form method="post">
								<input type="hidden" name="csrf_token" value="{$csrf_token}"><input type="button" class="button" onclick="form_admin.submit();" value="{"_SEARCH"|lang}"/></form></td> 
				</tr> 
				<tr> 
					...
					<td><form method="post">
								<input type="hidden" name="csrf_token" value="{$csrf_token}"><input type="button" class="button" onclick="form_server.submit();" value="{"_SEARCH"|lang}"/></form></td> 
				</tr> 
			</tbody> 
		</table> 
		...
		<table width="80%" cellpadding="2">
			...
			{foreach from=$ban_list_aktiv item=active_ban}
				...
				<tr class="list" style="cursor:pointer;" onClick="NewToggleLayer('layer_{$active_ban.bid}');">
					...
					<td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$active_ban.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td>
				...
				<tr id="layer_{$active_ban.bid}" style="display: none">
					<td colspan=10><div style="display: none">
							...
							<tr><td><b>{"_BANBY"|lang}:</b></td><td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$active_ban.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td></tr>
		...
		<table width="80%" align="center" border="1" cellpadding="2">
			...
			{foreach from=$ban_list_exp item=expired_ban}
				...
				<tr class="list"  style="cursor:pointer;" onClick="NewToggleLayer('layer_{$expired_ban.bid}');">
					...
					<td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$expired_ban.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td>
				...
				<tr id="layer_{$expired_ban.bid}" style="display: none">
					<td class="table_list" colspan=10><div style="display: none">
							...
							<tr><td><b>{"_BANBY"|lang}:</b></td><td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$expired_ban.admin_nick}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td></tr>
