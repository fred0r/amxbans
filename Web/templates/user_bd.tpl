		...
		<table frame="box" rules="groups" summary=""> 
			...
			<tbody> 
				...
				<tr>
					...
					<td class="fat">{"_BANBY"|lang}</td><td>{if $smarty.session.loggedin == "true" || $vars.show_admin_public == 1}{$ban_detail.admin_nick}{if $ban_detail.nickname} <i>({$ban_detail.nickname})</i>{/if}{else}<i>{"_HIDDEN"|lang}</i>{/if}</td>
